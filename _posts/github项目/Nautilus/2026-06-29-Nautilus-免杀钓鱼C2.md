---
layout: single
title: "Nautilus 免杀钓鱼C2 总览"
permalink: /Nautilus免杀钓鱼C2/
date: 2026-06-29
categories:
  - github项目
  - Nautilus
...
项目地址：<https://github.com/nk7667/Nautilus>

<br />

Nautilus 是一个 Go 语言编写的 C2（Command & Control）框架，包含服务端、植入体、分阶段加载器三个核心组件。支持 **LNK 链路** 和 **PDF 链路** 两条独立投递通道，均集成完整免杀技术栈。

项目核心目标：**在不依赖商业工具的前提下，实现并运用C2 的核心能力，并在国内主流沙箱达到极低检出率。**
...
| 链路         | 入口              | 流程                                                 | 诱饵                 |
| ---------- | --------------- | -------------------------------------------------- | ------------------ |
| **LNK 链路** | `challenge.lnk` | LNK → wscript.exe → update.vbs → payload + notepad | CTF\_challenge.txt |
...
| `assets\data` 目录名          | `__MACOSX` 是 macOS 压缩包特征，被 YARA 规则监控 | 避免触发 \_\_MACOSX YARA 规则 |
...
根据诱饵类型，也就是lnk快捷键伪装打开的内容，选择不同的伪造目标：
...
PDF 链路利用 Windows 双扩展名视觉欺骗（`简历.pdf.exe` 在部分电脑显示 `简历.pdf`）：
...
Nautilus 服务端提供 5 个功能 Tab 面板，所有操作通过 WebSocket 实时推送结果。
...
> 详细协议实现见 [Nautilus (一)：通信协议与数据包格式](2026-07-08-Nautilus\(一\)-通信协议深度.md)。
...
![VT检出率](/assets/images/nautilus-overview.png)
...
一个没有可疑特征的标准 PE 文件，比经过大量修改的异常 PE 更容易通过检测。
