# Nautilus C2 — 深度免杀实践：从16/71到7/71的对抗之路
...
Nautilus 是一个从零构建的轻量级红队 C2（Command & Control）框架，采用 Go 语言编写，涵盖服务端、植入体、分阶段加载器三个核心组件。项目核心目标：**在不依赖商业工具的前提下，实现前沿 C2 的核心能力，并通过分层免杀技术将 VirusTotal 检出率压缩至 7/71（90% 免杀率）。**
...
免杀是一个分层的系统工程，需要从**静态特征消除**、**运行时行为对抗**、**PE结构标准化**、**网络通信伪装**四个维度进行纵深防御。Nautilus 的免杀方案经历了四轮迭代，从初始的 16/71 VT检出逐步优化至 7/71。
...
**自洽闭环设计**：Unhooking 操作本身使用的 `NtProtectVirtualMemory` 和 `RtlCopyMemory` 全部通过 XOR 加密名称动态解析，Unhooking 过程不会被 hook 拦截。
...
**原理**：`EnumWindows` 和 `EnumChildWindows` 是标准的 Win32 GUI API，接收一个回调函数指针。操作系统在枚举窗口时自然地调用该回调——对杀软来说，这是一次完全正常的窗口枚举操作，而不是 shellcode 执行。回调函数指针通过 XOR 加密名称动态获取，静态分析无法发现其用途。
...
**核心原则**：免杀不是"越改越多"，而是"越像正常程序越好"。一个结构标准、签名完整、熵值合理的 PE 文件，比经过大量修改的异常 PE 更容易通过检测。
