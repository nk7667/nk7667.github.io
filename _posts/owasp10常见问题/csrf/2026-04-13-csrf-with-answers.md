...
- XSS 是在目标站点上下文中执行恶意脚本，重点是“执行脚本”
- CSRF 是诱导受害者浏览器向目标站点自动发请求，重点是“借身份发请求”
- XSS 往往还能进一步绕过 CSRF 防护，因为脚本可能读取页面中的 token

12. CSRF 的原理以及如何防范？

原理是浏览器会自动携带目标站点的 Cookie，攻击者利用这一点诱导用户访问恶意页面，从而触发敏感操作。常见防护包括：CSRF Token、SameSite Cookie、关键操作二次验证、校验 Origin/Referer、避免重要接口只依赖 Cookie 身份。
