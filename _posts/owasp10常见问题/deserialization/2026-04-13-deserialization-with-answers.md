...
### 4. 反序列化链里，哪些魔术方法/触发点值得重点找？

PHP 里常重点看 `__wakeup()`、`__destruct()`、`__toString()`、`__call()`、`__invoke()` 等能在反序列化后自动或间接触发的方法；Java 里则常关注 `readObject()`、`readResolve()`、`finalize()`、比较器、Transformer 链、JNDI lookup 等触发点。找链时就是找“可控数据 -> 魔术方法/回调 -> 危险函数”这条路径。

### 5. 遇到 Shiro、Fastjson、WebLogic 站点时，排查思路是什么？

先识别技术栈和版本，再判断是否存在公开高危链路：
- Shiro：看 `rememberMe`、默认 key、550/721 等利用条件
- Fastjson：看版本、是否开启 AutoType、是否存在 JNDI/gadget 利用面
- WebLogic：看未授权访问、任意文件上传、反序列化、控制台暴露等历史高危点
确认版本和利用前提后，再决定是走漏洞验证、流量特征排查，还是补丁与配置加固。

### 6. 反序列化常见函数有哪些？

- PHP：`serialize()`、`unserialize()`
- Java：`ObjectOutputStream.writeObject()`、`ObjectInputStream.readObject()`
真正危险的是对不可信输入调用反序列化入口，并且链路中存在可达的危险方法。
