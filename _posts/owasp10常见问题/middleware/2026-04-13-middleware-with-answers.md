...
27. Web 容器（中间件）常见解析漏洞有哪些？

常见有 IIS 5/6 目录解析、IIS 7/7.5 与部分 Nginx 的解析链问题、Nginx 早期空字节问题、Apache 多后缀解析等。这类漏洞通常与文件上传配合，最终目标是把原本不可执行的文件变成可执行脚本。

28. WebLogic 常见漏洞类型有哪些？

常见高危方向包括未授权访问、任意文件上传、反序列化、控制台暴露、组件 RCE 等。排查时通常先看版本、补丁情况、控制台是否开放，再结合历史漏洞做针对性验证。
