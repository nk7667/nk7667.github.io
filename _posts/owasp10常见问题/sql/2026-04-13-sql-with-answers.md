...
#### 26. SQL 注入从代码层面看，成因是什么？

本质是把用户输入直接拼接进 SQL 语句，让输入既当“数据”又参与“语法”。常见场景包括字符串拼接、MyBatis `${}` 直拼、动态排序字段直接拼接、二次注入等；如果没有参数化查询、白名单和类型约束，就会形成注入。

#### 27. SQL 注入常见分类、存在点、判断与利用思路是什么？

常见分类有报错注入、布尔盲注、时间盲注、联合查询、堆叠注入、二次注入、宽字节注入。存在点通常在查询参数、搜索框、登录框、排序字段、过滤条件、批量接口、ORM 动态 SQL 拼接位置。判断时先看是否闭合、是否报错、列数是否可控，再区分数据库类型与注入方式；利用时再根据是否有回显，选择联合查询、盲注、报错或带外通道。

#### 28. SQL 注入常用工具、手工结合工具时怎么做？

常用工具是 sqlmap，也会配合 BurpSuite 手工测参数、改包、重放、确定闭合和注入类型。一般先手工判断是否存在注入、数据库类型、列数和回显位置，再交给工具跑，提高准确性并减少误报。

#### 29. Oracle、MSSQL、MySQL 注入利用的差异有哪些？

差异主要在函数、系统表、报错方式、文件读写和命令执行能力上：
- MySQL 常见 `@@version`、`database()`、`load_file()`、`into outfile/dumpfile`
- MSSQL 常见 `xp_cmdshell`、`xp_dirtree`、`sysobjects`
- Oracle 常见 `dual`、`UTL_HTTP`、`DBMS_LDAP` 等带外能力
所以实战里要先判断数据库类型，再选 payload 和利用方向。

#### 30. 注入拿 Shell 一般要满足什么条件？

需要满足数据库用户权限足够高、目标环境允许写文件或命令执行、能确定写入路径或 Web 根目录、目标中间件会解析写入文件。MySQL 常见是写 WebShell，MSSQL 常见是命令执行或借系统组件落地。
