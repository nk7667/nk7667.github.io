...
- 云元数据：`http://169.254.169.254/`（高危）
- 本地文件/伪协议：`file://`、`gopher://`、`dict://`、`ldap://` 等（取决于客户端支持）

### SSRF 修复/缓解要点

- **域名白名单**：只允许请求固定业务域名（不要做黑名单）
- **协议白名单**：只允许 `http/https`，拒绝 `file/gopher/dict/ftp/ldap` 等
- **解析后校验最终 IP**：做 DNS 解析，校验不落到内网/保留网段/本地回环/链路本地；并在**每次重试/每次重定向**后再次校验
- **禁止或限制重定向**：避免 30x 跳内网绕过
- **出网隔离**：将发起请求的服务放到受限网络（禁止访问内网与元数据）
